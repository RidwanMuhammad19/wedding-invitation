...
import { EPOCH_EVENT_RESEPSI } from '@/constants';
...
function useDateCountdownResepsi() {
  ...
  const calculateTimeLeft = useCallback(() => {
    const TIME_LEFT = EPOCH_EVENT_RESEPSI - currentEpochTime;

    if (TIME_LEFT < 0) {
      setIsEventOver(currentEpochTime > EPOCH_EVENT_RESEPSI);
...
export default useDateCountdownResepsi;
